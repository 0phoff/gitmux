...
function main {
    ...
    if [ $? -eq 0 ]; then   # Git repo
        local git_fields = ($("$SCRIPT_DIR/../scripts/gitstatus.sh" "$dir" 2>/dev/null))
        tmux setenv GITMUX_REPO             1 
        tmux setenv GITMUX_BRANCH           "${git_fields[0]}"
        tmux setenv GITMUX_REMOTE           "${git_fields[1]}"
        tmux setenv GITMUX_COMMITS_AHEAD    "${git_fields[2]}"
        tmux setenv GITMUX_COMMITS_BEHIND   "${git_fields[3]}"
